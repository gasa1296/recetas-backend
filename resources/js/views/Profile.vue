...
async function handleUpdate() {
    ...
    try {
        ...
        error.value = (err as any).response?.data?.message || 'Error al actualizar el perfil'
    } finally {
        loading.value = false
    }
}
...
<template>
    <div>
        <h1 class="text-2xl font-bold text-brand-primary mb-6">Perfil</h1>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <div class="space-y-2">
                        ...
                        <button type="button" @click="addPhone" class="text-sm text-indigo-600 hover:underline">+ Agregar teléfono</button>
                    </div>
                </div>
            </div>
            <div v-if="form.specialty" class="border-t border-slate-200 pt-4 mt-4">
                <h3 class="text-lg font-semibold text-gray-800 mb-3">Especialidad</h3>
