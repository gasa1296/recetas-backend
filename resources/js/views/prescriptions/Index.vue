            ...
            <table class="w-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg">
                ...
                <tbody>
                    <tr v-for="p in prescriptions" :key="p.id" class="border-b border-gray-100 dark:border-gray-800 last:border-0">
                        ...
                        <td class="px-4 py-3">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium" :class="p.status === 'finished' ? 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300' : 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/50 dark:text-yellow-300'">
                                {{ p.pretty_status || p.status }}
                            </span>
                        </td>
                        <td class="px-4 py-3 text-gray-600 dark:text-gray-400 max-w-xs truncate">{{ p.diagnostic }}</td>
                        <td class="px-4 py-3 text-right whitespace-nowrap">
                            <router-link :to="{ name: 'prescriptions.edit', params: { id: p.id } }" class="text-sm text-indigo-600 dark:text-indigo-400 hover:underline mr-3">Edit</router-link>
                            <button v-if="p.status !== 'finished'" @click="finishPrescription(p.id)" class="text-sm text-green-600 dark:text-green-400 hover:underline mr-3">Finish</button>
